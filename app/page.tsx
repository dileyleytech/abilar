...
import { IconFoto, IconConversas, IconObra, IconSeguro, IconOk, type LucideIcon } from '@/components/ui/icons';
...
const STEPS: { Icon: LucideIcon; title: string; desc: string }[] = [
  { Icon: IconFoto, title: 'Você mostra o espaço', desc: 'Envie uma foto do cômodo e as medidas. Sem complicação.' },
  { Icon: IconConversas, title: 'A ABI te mostra', desc: 'Converse e veja uma prévia do móvel no seu ambiente, do seu jeito.' },
  { Icon: IconObra, title: 'O marceneiro realiza', desc: 'Receba orçamentos de profissionais da sua região e contrate com segurança.' },
];
  ...
  return (
    <div className="flex min-h-screen flex-col bg-base">
        ...
        <div className="mx-auto grid w-full max-w-screen-2xl items-center gap-10 px-4 py-16 sm:px-6 lg:grid-cols-2 lg:py-24 lg:px-8">
          <div className="flex flex-col items-start gap-6">
            <span className="inline-flex items-center gap-2 rounded-pill bg-brand-secondary/12 px-4 py-1.5 text-sm font-medium text-brand-secondary">
              <IconSeguro size={18} aria-hidden /> Pagamento protegido por etapa de obra
            </span>
            ...
            <div className="flex flex-wrap gap-3">
              <Link href="/cadastro" className={buttonVariants({ variant: 'primary', size: 'lg' })}>
                Começar meu projeto
              </Link>
              <Link href="/cadastro" className={buttonVariants({ variant: 'outline', size: 'lg' })}>
                Sou marceneiro
              </Link>
            </div>
          ...
          <div className="relative flex justify-center lg:justify-end">
            <div className="relative w-full max-w-md rounded-3xl border border-subtle bg-surface p-8 shadow-lg">
              ...
              <p className="mt-3 flex items-center justify-center gap-1.5 text-center text-sm text-muted"><IconOk size={16} aria-hidden /> Prévia do seu móvel, antes de contratar</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {STEPS.map((s, i) => (
            <div key={s.title} className="rounded-2xl border border-subtle bg-surface p-7 shadow-sm">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-deep text-brand-primary"><s.Icon size={26} aria-hidden /></div>
      ...
      <section className="mx-auto w-full max-w-3xl px-4 py-20 text-center sm:px-6 lg:px-8">
        ...
        <Link href="/cadastro" className={buttonVariants({ variant: 'primary', size: 'lg', className: 'mt-6' })}>
          Criar conta grátis
        </Link>
      </section>
