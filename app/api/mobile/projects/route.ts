...
import { activeArchitectById } from '@/lib/architects/queries';

// Cliente cria um pedido (RASCUNHO) com nome + local (cidade/CEP). Os móveis e a
// publicação vêm depois — mesmo fluxo da web. JSON: { title, city?, cep?, lat?, lng?, architectId? }.
export async function POST(req: Request): Promise<Response> {
  const auth = await authenticateBearer(req);
  if (!auth) return json({ error: 'Não autenticado.' }, 401);

  const b = (await req.json().catch(() => ({}))) as Record<string, unknown>;
  const p = createProjectSchema.safeParse({ title: b.title, sourceType: b.sourceType, city: b.city, cep: b.cep, lat: b.lat, lng: b.lng, architectId: b.architectId });
  if (!p.success) return json({ error: p.error.issues[0]?.message ?? 'Dados do pedido inválidos.' }, 400);

  // No mobile não há cookie de indicação — só a escolha explícita (autocomplete).
  const architectId = p.data.architectId ? await activeArchitectById(p.data.architectId) : null;
