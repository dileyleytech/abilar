...
import { runTurnOnState } from '@/lib/design/core';

// POST { projectId, state, utterance } → interpreta sobre o estado FORNECIDO (cópia
// do marceneiro), sem persistir. O marceneiro precisa ter orçamento no projeto.
export async function POST(req: Request): Promise<Response> {
  const auth = await authenticateBearer(req);
  if (!auth) return json({ error: 'Não autenticado.' }, 401);

  const body = (await req.json().catch(() => null)) as { projectId?: string; state?: DesignState; utterance?: string } | null;
  ...
  const r = await runTurnOnState(body.state, body.utterance);
